	...
	for (int i = 0; i < opt->NumberOfRvaAndSizes; i++)
	{
		...
		case IMAGE_DIRECTORY_ENTRY_EXPORT: {
			...
			for (size_t i = 0; i < Export->NumberOfNames; i++)
			{
				...
				UnDecorateSymbolName(namebegin, arr, 512, 0);
		...
		case IMAGE_DIRECTORY_ENTRY_IMPORT: {
			...
			while (Import->Name>0 && Import->FirstThunk > 0)
			{
				std::cout << filedata + RVAToOffset(Import->Name, filedata) << std::endl;
				int ooffset = 0;
				IMAGE_THUNK_DATA *IAT = (IMAGE_THUNK_DATA*)(filedata + RVAToOffset(Import->FirstThunk+ooffset, filedata));
				while (IAT != NULL && IAT->u1.Function > 0 && IAT->u1.AddressOfData > 0){
					if (IAT->u1.Ordinal > LONG_MAX){
						//最高位1 lower 序号
						int idx = LOWORD(IAT->u1.Ordinal);
					}
					else{
						//最高位0 
					}
					IMAGE_IMPORT_BY_NAME *IBN = (IMAGE_IMPORT_BY_NAME*)(filedata + RVAToOffset(IAT->u1.AddressOfData, filedata));
					if (IBN != NULL){
						char *pIBN = IBN->Name;
						std::cout << "function:   " << pIBN <<  std::endl;
					}
					ooffset += sizeof(IMAGE_THUNK_DATA);
					IAT = (IMAGE_THUNK_DATA*)(filedata + RVAToOffset(Import->FirstThunk + ooffset, filedata));
				}
				offset += sizeof(IMAGE_IMPORT_DESCRIPTOR);
				Import = (IMAGE_IMPORT_DESCRIPTOR*)(filedata + offset);
			}
		}break;
		case IMAGE_DIRECTORY_ENTRY_RESOURCE:{//资源表
			/*int offset = RVAToOffset(dire.VirtualAddress, filedata);
			IMAGE_IMPORT_DESCRIPTOR *Import = (IMAGE_IMPORT_DESCRIPTOR*)(filedata + offset);*/
		}break;
		case IMAGE_DIRECTORY_ENTRY_EXCEPTION:{//异常表
		}break;
...
static char *getdata(std::string path) {
	...
	std::cout << "文件大小：" << size1 << " 申请大小:" << size1 + 100 << std::endl;
